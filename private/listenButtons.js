...
export function listenCreateButtons() {
  document
    .querySelector(".create .next-round")
    .addEventListener("click", async () => {
      ...
      if (!params.has("participatePage")) {
        history.pushState(
          {},
          "Dashboard",
          `http://localhost:8080/index.html?createPage=${page}`
        );
      } else {
  ...
  document
    .querySelector(".create .previous-round")
    .addEventListener("click", async () => {
      const params = new URLSearchParams(window.location.search);
      let page = "1";
...
export function listenParticipateButtons() {
  document
    .querySelector(".participate .next-round")
    .addEventListener("click", async () => {
      const params = new URLSearchParams(window.location.search);
      let page = "2";
      ...
      if (!params.has("createPage")) {
        history.pushState(
          {},
          "Dashboard",
          `http://localhost:8080/index.html?participatePage=${page}`
        );
      } else {
  ...
  document
    .querySelector(".participate .previous-round")
    .addEventListener("click", async () => {
      const params = new URLSearchParams(window.location.search);
      let page = "1";
...
export function listenEditButtons() {
  const editCreatedButtons = document.querySelectorAll(
    "[class^='created_detail_']"
  );
  ...
  const editParticipatedButtons = document.querySelectorAll(
    "[class^='participated_detail_']"
  );
