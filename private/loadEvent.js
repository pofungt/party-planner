...
export async function loadCreateEvents(page) {
  ...
  const pageCreateContainer = document.querySelector(
    ".create .turn-page-button-container"
  );
...
export async function loadParticipateEvents(page) {
  ...
  const pageParticipateContainer = document.querySelector(
    ".participate .turn-page-button-container"
  );
...
export async function loadEventDetails() {
  ...
  const res = await fetch(
    `/events/detail/${isCreator ? "created" : "participated"}/${eventId}`
  );
  if (res.status !== 200) {
    const data = await res.json();
    alert(data.msg);
  ...
  if (result.status) {
    ...
    if (result.detail.venue) {
      venueString = `
        <a href="https://www.google.com/maps/search/${result.detail.venue.replaceAll(
        " ",
        "+"
      )}/" target="_blank">
          ${result.detail.venue || ""}
...
export function loadParticipantsModal(currentList, deletedList) {
  ...
  const currentParticipantModal = document.querySelector(
    "#participants-modal #current-participants-list"
  );
  ...
  const deletedParticipantModal = document.querySelector(
    "#participants-modal #deleted-participants-list"
  );
